                    ...
                    for interface in value:
                        ...
                        if 'interface_name' in interface:
                            if interface2 is None:
                                for i2 in value2:
                                    if 'interface_name' in i2 and i2['interface_name'] == interface['interface_name']:
                                        interface2 = i2
                                        break
                            assert interface2 is not None, f"Interface {interface['interface_name']} not found in other observation"
                        subnet = None
                        subnet2 = None
                        ...
                        if 'ip_address' in interface:
                            if interface2 is None:
                                ...
                                assert 'ip_address' in interface2
                            if subnet is not None:
                                assert interface['ip_address'] in subnet.hosts(), f"ip_address {interface['ip_address']} not in subnet {subnet}"
                                assert interface2['ip_address'] in subnet2.hosts(), f"ip_address {interface2['ip_address']} not in subnet {subnet2}"
                if key == 'Processes':
                    assert len(value) == len(value2), f"{value} != {value2}"
